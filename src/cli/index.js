...
async function main() {
  ...
  const commands = { init, status, ping, doctor, repair, update, reset };
...
async function doctor(cwk) {
  ...
  if (result.value.updateAvailable) {
    console.log(yellow('\nℹ Your runtime files were generated by an older CWK. Run: cwk update'));
  }
  if (!healthy) process.exitCode = EXIT.PROJECT;
}

async function update(cwk) {
  const result = await cwk.update();
  if (!result.ok) return fail(result.error);

  if (asJson) return console.log(JSON.stringify(result.value, null, 2));

  const { changes } = result.value;
  console.log(bold('CWK Update') + '\n');

  if (changes.length === 0) {
    return console.log(`${green('✓')} Already up to date.`);
  }
  ...
  console.log(green(`${changes.length} change${changes.length > 1 ? 's' : ''} applied.`) + ' Commit and push them to activate the new runtime.');
}
...
Commands:
  ...
  update    Bring an existing project to the current CWK format
