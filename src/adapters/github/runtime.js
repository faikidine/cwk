...
export class GitHubActionsRuntime {
  ...
  /**
   * Does the workflow match what the current CWK version generates?
   * The comparison regenerates the expected content with the file's
   * own cron minute, so user alignment never counts as a difference.
   */
  async outdated() {
    let content;
    try {
      ...
      return { outdated: true, reason: `The workflow (${WORKFLOW_PATH}) was missing.` };
    }

    const match = content.match(/cron:\s*'(\d{1,2}) \* \* \* \*'/);
    const cronMinute = match ? Number(match[1]) : 0;
    if (content === createGitHubActionsWorkflow({ cronMinute })) {
      return { outdated: false };
    }
    return { outdated: true, reason: 'The workflow did not match the format generated by this version of CWK.' };
  }
