...
Brings an existing project up to the current CWK format.

The engine logic itself updates automatically (the runtime installs the
latest CWK on every run). `update` takes care of what lives in the
user's repository and cannot update itself:

- runtime files generated by an older CWK (regenerated to the current
  format, preserving user-specific values such as the cron minute);
- configuration fields introduced by newer versions (added explicitly
  with their default values);
- project metadata (`cwkVersion`, future `formatVersion` migrations).

`update` never touches anything that is already current.
...
⚠ Runtime
  was: The workflow did not match the current format.
  now: Regenerated .github/workflows/cwk.yml (kept your schedule minute 50).

⚠ Configuration
  was: patienceMinutes did not exist yet.
  now: Added it with the default (25 minutes).

2 changes applied. Commit and push them to activate the new runtime.
```
...
`update` expects a healthy project: corrupted or missing files are
`repair`'s job. Repair fixes what is broken; update modernizes what is
old.
